...
- `farloads/` — the pure-calc package. No Streamlit, no file access in calc code.
  ...
  - `workflow.py` — the ordered step graph (phases `Start → Develop V-n diagram →
    Flight loads → Other loads → Landing loads → Load-case plotting → Export`; pure
...
replicate the manual exactly (warts and all).

**The register of record — every approved deviation, with its rationale, figures
and citations — lives in
[`docs/20_theory/02_approved_corrections.md`](docs/20_theory/02_approved_corrections.md).**
Add each new correction there (and to the calc/test/spec/CHANGELOG as above). As of
this writing the register holds: **23.361(a)(1)** takeoff-torque factor,
**23.361(a)(3)** turboprop-malfunction mean-torque factor, and **23.427(a)**
unsymmetrical-tail candidate set.
