...
def run(project: Project) -> ModuleResult:
    ...
    for result, spec in zip(build_tabs(project), project.tab_loads.tabs):
        ...
        conditions.append(ConditionResult(
            ...
            values=[
                LoadValue("VC", vc, "kt(EAS)", key="vc"),
                LoadValue("Tab chord ratio E", r.chord_ratio, key="tab_chord_ratio_e"),
                LoadValue("Tab load", r.load_lb, "lb", key="tab_load"),
                LoadValue("Tab LE pressure", r.le_pressure_psi, "lb/in^2", key="tab_le_pressure"),
                LoadValue("Tab TE pressure", r.te_pressure_psi, "lb/in^2", key="tab_te_pressure"),
            ],
            note=note,
