...
| [`19_l7_lateral_body_aero_note.md`](30_future/19_l7_lateral_body_aero_note.md) | **Lumped lateral body aero design note** (decision **L-7**, backlog Pri 8) — the fin is the suite's only lateral aero: DATCOM 5.2.3.1/5.2.1.1 `Cn_β`/`Cy_β` transcribed from the Digital DATCOM source (bundled chart data, closed-form `K_Rl`, ex1–ex11 as a printed **oracle**), the verified FAR 23.441(a) text (§4 — (a)(2) is a 1.3× overswing, source of SELECT's 19.5°, and it licenses a *static-stability* gate rather than a net-moment one), the measured `concept_regional_jet` derivatives and case effects, decisions L-7.1…L-7.7 and gates G1–G13. **Revision 2** carries three marked corrections to revision 1, including the shipped `n_y`-direction defect it uncovered. **PROPOSED — not agreed, no code** |
