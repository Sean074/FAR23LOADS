...
| **Digital DATCOM** | `reference/datcom/` (added 2026-08-15 — see its `PROVENANCE.md`) | USAF Digital DATCOM, the computer implementation of the *USAF Stability and Control DATCOM* chart methods (public domain, via PDAS). Carries the chart data as `DATA` statements, so a method can be ported without the printed charts. Used for the lateral body derivatives of decision **L-7** — `Cn_β` (DATCOM 5.2.3.1, `datcom.f:33413-33444`, chart data `:28725-28756`) and `Cy_β` (5.2.1.1). **Its 11 sample cases carry printed `CYB`/`CNB` output and are the oracle for that port**; because `reference/` is gitignored, an oracle test carries the printed numbers as literals with this citation, exactly as the Appendix A tests do. |
