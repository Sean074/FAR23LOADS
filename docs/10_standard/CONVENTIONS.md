...
| **Which project-JSON field is which quantity** (the Project JSON Editor's SI view; name-keyed, so a force and a weight sharing the `_lb` suffix must be classified apart — the factors are ~9.8× different. A classified key converts whether its value is a scalar or a list of numbers. A dimensional-looking name that is *not* a quantity — a bool such as `override_max_continuous_hp` — is exempted with its reason, never left to fall through) | `units._PROJECT_FIELD_KIND` + `units._NOT_DIMENSIONAL` | `tests/test_project_units.py::test_every_dimensional_project_field_is_classified` — walks the **type graph** from `Project`, not the fixtures, so a field no project has set is still covered (how `thrust_lb` slipped: unclassified, unset everywhere, and lossless in round trip) + `::test_the_exempt_list_only_holds_fields_that_are_not_quantities` |
