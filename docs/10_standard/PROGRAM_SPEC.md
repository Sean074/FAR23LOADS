...
- **Notes:** Slipstream is the momentum-theory sub 500 (iterate `U1` to absorb 0.85·MAXHP); computed only when engine power is present. A disk that absorbs the power at no speed **refuses** (#33) — the search used to return its own guard value, a 100,000 ft/s slipstream, and amplify the flap load by its square. Knots→ft/s is the exact `constants.KT_TO_FPS` (1.68781; the suite's `1.15·88/60` was measured 2026-08-17 to be unnecessary for the p201 oracle — it survives only as `KT_TO_FPS_SUITE` for ENGLOADS' `VSF`, issue #26).
