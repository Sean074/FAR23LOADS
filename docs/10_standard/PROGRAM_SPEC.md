...
| `mass` (weight/CG + inertias) | WTONECG `build_mass`, via the Weight & Mass **Apply weight items** handler (M4-17a) | FLTLOADS (weight/CG); SELECT, ONENGOUT (inertia); `configuration.cg_estimate` ("Weight DB" branch); Landing Loads *view* — waterline seed only (**LANDLOAD itself reads `landing.cg_cases`, not `mass`**) |
...
| `landing` (gear geometry + load factor) | LGFACTOR (N returned on `LoadFactorResult`, not stored — M2R-4), direct gear-geometry input | LANDLOAD; reads `landing.cg_cases` (required, weight/CG), `geometry.wing` (area). The workflow step `requires=()` (M4-17a) — it consumes no upstream slice |
