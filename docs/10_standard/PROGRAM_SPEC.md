...
- **Derive-by-default (note 36 OV-7, #97):** `engine.effective_engine`/`resolved_engines` resolve each engine onto a local copy — a blank `limit_load_factor` derives from the FAR 23.337 limit (`design_speed_values(project).n`; a 0 LIMNZ silently zeroed every mount case, C210-41), and with an `engine_mass_item`/`prop_mass_item` selector set (schema v56) the engine/prop weight and CG falsy-derive from the named `weight.items` row (D-25 mass SSOT; matched with `same_name`, refused by name when no row matches, `engine_mass_row_mismatch` warns on a typed disagreement). Every engine consumer — the mount loads, the LRA beam model, WINGGEOM's engine stations, the nacelle sketch, OEI and hub thrust — reads through the one resolver.
