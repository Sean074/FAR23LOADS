...
- **Calc is LIMIT; rendered/exported output is ULTIMATE.** Modules return **limit** loads (the oracle figures), so the Appendix A/B regressions are unaffected. `report.py` and `export/sbeam_bridge.py` multiply the **load** quantities (forces/moments/pressures, never geometry/weights/inertias/load factors) by the case `safety_factor` to report **ultimate = limit × 1.5**. The per-case field is the hook for a future 14 CFR 25.302 / Appendix K probability-based factor (1.0–1.5); for now every case is 1.5 (incl. sudden engine stoppage). See `reference/14CFR_factor_of_safety.md`.
- **One CSV shape per module = load cases.** Each row is one structural load case: `ID`, `FAR §`, `Case description`, an `SF` column, application point `Loc X/Y/Z`, then the applied (ultimate) loads/moments with `ULT`-marked headers. This is exactly the `load_cases_to_rows` pattern engloads already established — generalize it, don't reinvent per module.
