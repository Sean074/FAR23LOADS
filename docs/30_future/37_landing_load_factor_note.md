...
**Status: AGREED 2026-08-27 (owner, in session — `CLAUDE.md` rule 1's
working-alone path), milestone 0.8.0; nothing built.** Owner rulings taken (in
...
| **LF-5** | **`N > L` is a hard refusal**, message naming both values and the resulting NLG. | With LF-4 the cap no longer keeps `NLG = N − L` positive. `k = nap/nlg` ([landing.py:173](../../sloads/modules/landing.py)) is then one keystroke from a `ZeroDivisionError` or a sign flip that would ship as a quietly negative reaction. This guard is the *only* remaining protection. |
