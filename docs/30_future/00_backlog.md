...
| 16 | **The suite-runtime revisit clause has tripped — split the whole-pipeline-per-assertion tests** *(CR-D-6, filed from #46; hygiene)* (#92) | The tests that re-run the whole pipeline once per assertion — `test_case_ids` plus the four ~20–37 s deliverable-units/balance tests share that one shape — reduced to one pipeline run per example with the assertions reading its result, as the 2026-08-16 case was fixed. Measured at the trip: 269 s parallel (671 s serial) against the clause's ~2 min, slowest test 62.3 s (`test_case_ids::test_the_index_row_states_the_condition_its_cards_were_computed_at`, the parallel critical-path floor) against its ~30 s, 15 tests over 16 s. **Re-measured 2026-08-26 (this branch, owner's machine) — the trip figure was a coverage-instrumented run, and the clause is tripped only under instrumentation.** Plain `pytest`, the documented local command with no `--cov`: **62.1 / 62.7 s** on two runs, **1 call over 16 s and none over 30 s**, slowest `test_deliverable_units::test_imperial_output_matches_the_frozen_baseline` at 12.3 / 25.6 s (that spread is the run-to-run noise on this machine, and it is wide). Under `--cov=sloads` with `COVERAGE_CORE=sysmon`, the way CI's coverage leg runs it: **265.6 s**, reproducing the trip's 269 s inside the noise, with **13 calls over 16 s and 8 over 30 s**, slowest the same frozen-baseline test at 51.9 / 71.3 s. The named critical-path floor has moved off the list: `test_case_ids::test_the_index_row_states_the_condition_its_cards_were_computed_at` measures **2.3 s** in isolation and enters the top 20 in neither mode, so its 62.3 s at the trip was instrumentation cost, not the test. **This re-reading poses a scoping question the owner has not answered:** the clause's thresholds are written against the local command, which now passes all three, so this row is either closed as no-longer-tripped or re-aimed at the coverage leg — whose 4½ min is the figure that actually gates the push to `main`. The shape the row names (whole pipeline re-run per assertion) is unchanged and still real either way; only its measured cost is in question. **No `slow` marker** — `00_program_overview.md` §Testing states why. Done when `--durations` puts the critical path back inside the clause's own thresholds; the stale claim on the standard page was already corrected with #46, so what is open here is the work, not the wording | V | S / S–M | — |
