...
| 15 | **Oracle results mask real errors — remainder** — a mid-entry planform crashes Wing Loads, the session disagrees with its own reloaded file *(PB-18, PB-21; defect)* (#71) | The aero `__post_init__` derivation runs at persist; planforms with fewer than two points raise `MissingInputError`. **The `ZeroDivisionError` half shipped in 0.7.2** with the row-counter fix (`_NOT_READY = (ValueError,)`, `oracle_app/results.py`). **Do not narrow further to `MissingInputError` only** — that was written here as the plan, tried in 0.7.2 and rejected: it fails `test_oracle_journey`, because a bare `ValueError` ("`EngineInput.cylinders` is required for a reciprocating engine") is the second half of the documented error contract (`00_program_overview.md` — input present-but-unusable) and *is* a not-ready condition. `ZeroDivisionError` was in neither half, which is why it alone came out (corrected 2026-08-25) | V | S / S | — |
