...
design notes per step (open ones here — 09/11/21/24/32/34/38; notes 35/36/37
...
instead of waiting for 0.9.0. **Cut 0.8.1 when band B1 is empty; band B2
(0.9.0) follows it.** Before it: **0.8.0 is cut** — `v0.8.0`, 2026-08-28,
...
# Priority table (re-cut 2026-08-28 — the single order of work)
...
| 1 | **The airplane-datum resolution of every attitude-1 ground case rotates the wrong way — the exported ground FORCE cards and gear reference-point loads carry it** — LANDLOAD.BAS adds the ground angle in PHIM/PHIN for the braked-roll, side and supplementary-nose families where the level and tail-down families (and physics) subtract it, an error of 2·GRA₂ ≈ 9.45° in body resultant direction; adjudicated against the manual's own p232 printout (level 13.921 and tail-down −15.000 subtract; ground roll 43.387 adds) **and confirmed verbatim in the Appendix C `.BAS` listing** (`PHIM(L)=ATN(.8)*57.3+BETA(2)`, note 38 §1.1). ga6 case 16 (2-point braked roll): vm/dm 2104/1989 as coded vs 2402/1616 corrected (**+14 % / −19 %**); the side family's body drag flips sign (+186 → −186, aft where nose-up geometry demands forward). Ground-line values and the p230/p231/p236 oracles untouched; the G-6 gate recovers ρ from the case's own resolutions so it structurally cannot catch this. **A first-order defect on shipped output — pulled into 0.8.1 under the 2026-08-24 ruling 1 (re-cut 2026-08-28); this row is the patch band's first act.** Design note **38** (GF-1…GF-5, G-GF-1…G-GF-5), AGREED required before code; approved-deviation entry per GF-3. **Supersedes the register's "Considered and declined" decision of 2026-08-15** ("replicate as printed", taken while p231–233 was OCR-garbled; its own text names legible output as the reopening condition — note 38 §1.2): the declined entry converts in place to an approved deviation, and its pin test `test_the_ground_roll_attitude_is_resolved_against_the_other_sign` is flipped to pin ρ = −GRA on every attitude, never deleted. **GF-1/GF-2 REOPENED 2026-08-28** — the correction was written, measured and reverted: it reproduces every §1.3 figure and leaves the p230/p231/p236 oracles green, then moves the assembled ground cases from 1e-5 to 0.106·W·MAC against LANDLOAD's own unbalanced pitching moments, an independent reference the note assumed did not exist (note 38 §1.8). The fix site is the hole: `_geometry` builds the p230-locked lever arms from the same `beta` GF-2 rules untouched. **OQ-2/OQ-3 since resolved from the Appendix C listing (note 38 §1.9): GF-1 is re-confirmed and GF-2 refuted** — correcting the arms *and* PHIM/PHIN together drives the closure residual to ~0, better than the printed numbers, and costs exactly one printed table (the p230 ground-roll AP/BP/DP row, values recorded). **OQ-5 answered 2026-08-28 (§1.10): all three arms flip, CP does not** — `FNBP` simplifies to a projection, and `DP` settles the sign with no frame argument at all (the wheelbase between two contact points is 94.811 where p230 prints 93.147). **GF-2′ agreed (five tables, one sign); GF-3′ then WITHDRAWN (§1.11):** implemented and measured, the correction drives case 13's residual pitch from −757.1 to −0.7 lb-in and passes the closure gate untouched, but the arms feed the reaction solve, so it also moves printed **p231** reactions — +7.7 % on cases 13–15 VMP, **−40 % on the supplementary-nose VNP** — a far larger deviation surface than the agreed scope. **Gate: GF-3″**, a register entry stating the whole surface with every deviated-from value transcribed from the rendered page. No code until then; the register's declined entry is **not** superseded meanwhile. **§1.13 (2026-08-29): the sub-finding is CLOSED and the gate's inputs are ready.** p231, p232 and p233 read cleanly rendered at 200 dpi — garbled by OCR, never illegible — and every printed cell of all three now locks against the port at the page's print resolution, all 33 cases, both frames, factors and unbalanced moments. GF-3″'s deviated-from set is therefore transcribed and executable rather than computed from the pre-fix code. Two consequences for the ruling: **GF-1 and GF-2′ no longer cost the same thing** — GF-1 moves only p232, while GF-2′ additionally departs from p231 cases 13–15/25–33 and p233 cases 16–24, all of which match the port today, so the register must price them separately; and p233 prints a **second ground-to-datum rotation** (`RMOM = RMOMP·cos GA + YMOMP·sin GA`) applied with the *same* sign on **every** attitude, a **third instance** of the sweep class after PHIM/PHIN and the ND lift term. GF-1 itself is unchanged and now rests on the tail-down row alone, which needs no consistency argument: there θ = 0, so PHIM *is* the ground normal in body axes and prints −15.000 = −GRA₃ (#133) | PHIM/PHIN corrected at the tables only (fix site GF-2, lever arms and primed set untouched); the p232 level/tail-down rows become new oracle locks; `02_approved_corrections.md` declined entry converted, recording the deviated-from printed values; the absolute ρ == −GRA gate (GF-4) replaces the self-consistent check's blind spot; assembled ground cases and sbeam round-trip re-verified on existing CI gates | E | L / M | design note 38 AGREED |
| 2 | **The landing output is single-frame and unlabeled where the original prints two tables** — `run()` ships only the ground-line set: `vm/dm/vn/dn` exist on `GearReactionCase` but never reach `ModuleResult`, no per-case fuselage-axis angle is emitted (p231 prints one per family), the p232 airplane-datum load factors NR/NV/ND are not computed, no application point is identified (patch, strut state and reference node live only in the gear free-body report and the deck), and the Oracle's per-case tables name no frame while the deck consumes the other one. Design note **38** (GF-6/GF-7, G-GF-6); OQ-1 resolved 2026-08-28 from the Appendix C `.BAS` — the datum factors carry a rotated lift term whose sign is a second instance of the #133 error (note 38 §1.6), so they build with the corrected sign and their p232 cells join the GF-3 register (#134) | Each case emits both frames plus the fuselage-axis angle (`deg`, never SF-scaled); both GUIs caption every reactions table "with respect to ground line" / "with respect to airplane datum" from one caption owner with a drift guard; NR/NV/ND per OQ-1's resolved formula with the corrected lift sign (case 1 locks at 3.269/3.216/0.585; wheels-only NR as printed, case 16: 1.703); each case also states its application point and attitude. **Ships with or after the row above — never before** (GF-6) | V | M / S–M | the defect row above |
| 3 | **flight_envelope view crashes on a blank `full_down_aileron_deg` — the note-36 derive default is unrenderable in the main GUI** — `select_input.full_down_aileron_deg` defaults to `None`, a documented, meaningful state (the registry's sentinel-default class, #98/C210-49) the oracle GUI renders and SELECT derives, but the view dies with `TypeError: float(...)` on `NoneType`; baron_58 enters 14.0 (= its aileron limit) to keep the suite green, so **no shipped fixture exercises the blank and no guard covers it** *(found 2026-08-27 building the guide's baron_58, #96 stage 2)* (#121) | The view renders the blank as the oracle GUI does (empty widget + the derived value stated) or at minimum survives it; the rule-4 sweep of `app/views/` for the same `float(field)` pattern over other sentinel-default registry fields; the blank-field case added to the views dirty-flag/smoke sweep. **The `app/views/` freeze stays shut:** 0.8.1 takes the *at minimum survives it* half — the named refusal, the rule-4 `float(field)` sweep and the smoke case — and leaves the render-it-as-the-oracle-GUI-does half to #29's freeze lift, where the layout question belongs (re-cut 2026-08-28) | V | S / S | — |
| 4 | **A blank `limit_load_factor` derives through a half-entered planform and escapes as a raw `TypeError` instead of a named refusal (#71 class)** — `engine.effective_engine` derives blank LIMNZ from the FAR 23.337 limit through `design_speed_values(project)` (note 36 OV-7), and with the planform half-entered (the #71 mutation set) the derivation escapes as a traceback; every shipped fixture enters LIMNZ explicitly, so **the derive path is unexercised by any shipped fixture** *(found 2026-08-27 building the guide's baron_58, #96 stage 2)* (#122) | The OV-7 derive chain refuses by name when its planform inputs are unresolvable (the same statement the direct consumers make); the #71 mutation guard gains a fixture-independent blank-LIMNZ case over the mutation set — a property of the resolver, not of one field (rule 4) | V | S / S | — |
