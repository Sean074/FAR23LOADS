...
| 24 | **Composite fields can never show their non-owner mark; a result block with rows and no download raises** *(code review 2026-08-24 §4.3; both latent — real mechanisms, no live trigger)* (#89) | `_copy_note` is reachable only from `render_scalar`, so the first non-owner **composite** field ships unmarked and silently editable — the #36/CR-A-2 defect returning through a door never closed (zero such registry rows today, verified): a drift guard asserting every non-owner field in `oracle_input_paths()` is one the renderer can mark. Plus `st.columns(len(block.artifacts))` guarded for the empty case, which would crash a page that has a table and no download | V | S / S | with #73 |
