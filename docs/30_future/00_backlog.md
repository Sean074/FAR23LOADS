...
resolve in the review document. **The 0.6.0-candidate review
([`../50_reviews/2026-08-15_review_0_6_0_candidate.md`](../50_reviews/2026-08-15_review_0_6_0_candidate.md))
is fully closed**: its correction rows were added 2026-08-15 and every one of
them — the currency batch (R6-D1–D4), the three deliverable corrections
(R6-C1/C2/C3 with C4 folded in), the rotational gate half (R6-T1/T2) and the
three documentation-coverage findings (R6-D6, R6-D7, R6-D5) — closed the same
day, so nothing of the block remains here per the removal rule; the trails are
in the changelog and the history file. The table was renumbered sequentially in
the same cycle, discharging that review's R6-D8 as table mechanics rather than a
promoted item. The earlier ordering rules (2026-08-09: wrong
...
| 4 | The aileron's own lift increment is not distributed | `ACRL` wing cards gain the aero half of the couple (acts ~70 % span) | V | L / M | pairs plan 09 spanwise work |
| 5 | **Non-wing drag has no carrier in the assembled model** (*diagnosed 2026-08-15; was "RJ pitch-gate exceedance diagnosis"*) — design note in [`20_body_drag_carrier_note.md`](20_body_drag_carrier_note.md) (**proposed, awaiting agreement**) | A `body-axial` load — closes the `nx` gap in full and takes the pitch residual to the R3 floor; plan 13 G9 inherits the ceiling until then | V | L / M | — |
...
| 12 | Load-application axis vs elastic axis — document the torsion reference | Convention in `coordinates.py` + spec + deck `$` header | V | S / S | partially superseded by Pri 1 (LRA import) |
| 13 | Gust spanwise-distribution decision | Study + recorded decision | V | S / S | — |
| 14 | M4-19 — Multhopp distributed fuselage `Cm` | A distributed body pitching load for `body_loads` | V | L / M | — |
| 15 | M4-21 — fuselage pitching load factor | d'Alembert pitch term at each body station | V | M / S | pairs M4-4 |
...
| 19 | F25-0 — verify pass | Current CFR text for every *(verify)* row | V | S / S | precedes any F25 build step |
| 20 | Mach-margin route for the FAR 23 categories | Category gate + per-category default | V | S / S | — |
| 21 | Flutter-clearance Mach basis for transports | Verified reference + decision, then an opt-in variant | V | S / S | — |
| 22 | Upset-criterion speed increase (25.335(b)(1)) | The 20 s dive integration the margin route lacks | V | M / M | — |
| 23 | F25-1 — transport category "T" envelope pack | 25.337 floors, 25.341 U_ref schedule, VB | V | M / M | — |
| 24 | F25-4 — ground-loads parameter variant | 10/6 fps, lift = W, LDW/MTOW pairing | V | M / M | coordinates with M4-6 (shipped 2026-08-15) |
| 25 | Deliverables render structural negative zeros *(new 2026-08-11, from the body-deck signed-zero fix)* | `-0.000000E+00` components (~2,000 in one balanced deck) and the tail span CSV's `Fax` column normalised at the formatting boundary | V | S / S | schedule with a digest wave — cosmetic, and it moves every deck family's bytes, so it needs a wave of its own (the R6-C3 wave it was a candidate passenger on landed 2026-08-15 without it, by user decision) |
...
| 27 | Combined flight + ground station envelope *(new 2026-08-14, from step 10 decision G-9)* | Two-sided max/min per station over both families, each extreme naming its governing case | V | M / M | — (ground cases shipped 2026-08-15) |
| 28 | `applicability`/`direct_totals` design-weight re-point *(new 2026-08-14, from step 10 decision G-14)* | The FAR 23 gate reads the MTOW SSOT instead of the database total; `direct_totals` renamed | V | S / S | — (the G-14 hop shipped 2026-08-14) |
| 29 | M4-8 **Layer 2** — agreed named failure-case factors (25.302) *(remainder after Layer 1 shipped 2026-08-14)* | A named `25.302` failure case as its own governing row + ULTIMATE load case, and the system-reliability requirement it levies | V | M / M | coordinates with Phase F25 |
| 30 | `dhc8_dash8` gear mass is tagged `fuselage` but the leg is wing-carried *(new 2026-08-14, from step 10 decision G-2's carrier↔mass guard)* | Re-tag the gear items `MassComponent.WING` and re-pin `mass_distribution.wing_mass_tie` | V | S / S | — (the guard that found it shipped 2026-08-14) |
...
### [V] Non-wing drag has no carrier in the assembled model *(new 2026-08-08 as "the regional jet's low-CL balanced cases exceed the 1 % pitch gate"; **cause measured 2026-08-15**)*

**Diagnosis (2026-08-15, the element-count study this item asked for).** The
exceedance is neither of the two causes originally proposed. **The assembled
model has no load carrying the airplane's non-wing drag**, and the couple that
omission leaves about the CG *is* the pitch residual. The measurement is in
"What the study measured" below; the observation history that led to it is kept
after it, with its superseded hypothesis marked. Tier M. Effort: M.

*What ships:* a `body-axial` load, `dx - SumFx_wing`, applied on the body. It
closes the `nx` half of the same defect (below) in the same change. **Design
note: [`20_body_drag_carrier_note.md`](20_body_drag_carrier_note.md)** — and note
its §3: the placement is *not* obvious, because putting the load at the body's
mass centroid makes the pitch residual **worse** (the trim lumps all drag at the
wing plane, and the trim is oracle-locked). The note's D-1 is the decision that
settles it.
...
**[SUPERSEDED 2026-08-15]** The paragraph that stood here proposed two candidate
causes — "the strip-quadrature-vs-closed-form lift floor (R3), and the lumped
Munk term itself (**M4-19** ... the natural pairing)" — and the test to separate
them. The test was run and **both are refuted**; see below. The correlation with
the lumped `Cm` recorded above is real but not causal: `fuselage_cm` and the
residual both track low-CL/high-speed, which is the third variable.

#### What the study measured (2026-08-15)

**1. Element count settles nothing — R3 is out.** Sweeping
`SurfaceInput.elements` 5 → 640 (128×) on both fixtures, the pitch residual
converges by ~20 elements onto a **non-zero plateau**:

| RJ case | 5 | 10 | 20 | 80 | 320 | 640 |
|---|---|---|---|---|---|---|
| PLAA | 1.156 | 1.042 | 1.041 | 1.041 | 1.041 | **1.041** |
| TORS | 1.177 | 1.174 | 1.174 | 1.174 | 1.174 | **1.174** |
| SIDE GUST | 1.589 | 1.586 | 1.586 | 1.586 | 1.586 | **1.586** |

Quadrature error is the 5 → 20 movement, at most 0.115 pp and visible on PLAA
alone. It is not the exceedance.

**2. The residual is an exact three-term identity.** `fuselage_cm` is defined as
`vn.m_wf - wing_about_ac` about `(xw, zw)`, so `wing_about_ac` cancels
identically; the wing and body inertia moments are equal and opposite (±1.529 %).
Subtracting the trim's own balance (`flight_envelope._balance`) from the
assembled sum leaves, exactly:

    residual_My = (zw - zcg)*(SumFx_wing - dx)     drag-model gap x vertical arm
                - (xw - xcg)*(SumFz_wing - lz)     lift-model gap x horizontal arm
                - (xtc - xt)*lt                    tail-station gap x tail load

It closes to the last printed digit on every case:

| RJ case | drag term | lift term | tail-x term | sum | reported |
|---|---|---|---|---|---|
| PHAA | −0.528 % | +0.086 % | 0.000 % | −0.442 % | −0.442 % |
| PLAA | **+1.049 %** | −0.007 % | 0.000 % | +1.041 % | +1.041 % |
| PMAA | **+0.998 %** | −0.031 % | 0.000 % | +0.967 % | +0.967 % |
| TORS | **+1.151 %** | +0.022 % | 0.000 % | +1.174 % | +1.174 % |
| SIDE GUST | **+1.544 %** | +0.042 % | 0.000 % | +1.586 % | +1.586 % |

The pitch residual is the **drag** term, essentially in full. Lift never exceeds
0.086 %. The tail-station term is exactly zero on a clean configuration and can
only bite on a flaps-down point, where `_balance` switches to `xtf` while
`assemble` still applies the tail load at `xtc` — worth its own guard when
flapped V-n points are assembled.

**3. The cause.** The trim balances the **airplane's** drag from the polar
(`drag_cd(config, cl)`) at the wing AC height; the assembly has only the wing
strips' own chordwise force from `air_load_distribution`. Nothing carries the
rest:

| RJ case | `SumFx` strips | trim `dx` | gap |
|---|---|---|---|
| PHAA | −21,701 | −24,348 | +2,646 (11 %) |
| PMAA | −4,480 | +1,247 | −5,726 (459 %) |
| SIDE GUST | +1,800 | +4,885 | −3,085 (63 %) |

**Why the RJ and not ga6:** the arm `zw - zcg` is **−16.70 in** on the RJ against
**−5.27 in** on ga6 — 3.2× — on top of a larger non-wing drag share. **Why it
tracks the low-CL cases:** at high CL the wing's induced drag dominates and the
gap is 4–11 %; at low CL the parasite drag the strips do not carry dominates and
the gap is 63–459 %.

**4. M4-19 cannot fix this, and never could.** Beyond the measurement there is a
structural reason: `fuselage-cm` is applied as a **free couple**, so redistributing
it over body stations preserves its resultant exactly and cannot change
`residual_My` by any amount. The M4-19 pairing is dropped from this item.

**5. The same gap, in `x`.** `residual_fx` equals `SumFx_wing` exactly — nothing
else in the assembled model carries `fx` — so ga6 PHAA's closure `nx` of 0.661
against the trim's drag 0.610 is this same gap, and it is likewise
element-independent (−191.5 → −173.4 lb over 5 → 640, converged). Per
`CLAUDE.md` practice 4 the fix covers both degrees of freedom in one change.

**6. "Strip quadrature" was the wrong name for the force floor too.** The `Fz`
residual also converges to a non-zero plateau (ga6 PHAA −34.6 lb at the default
20 elements, **−42.3 lb / 0.327 % converged**); at 20 elements the quadrature
transient partly cancels the model gap. The floor is the strip lift
distribution's integral against the trim's closed-form `CL*q*S` — a model
difference, not a quadrature error. Corrected wherever it was stated
(2026-08-15); the identification was right, the name was not.
