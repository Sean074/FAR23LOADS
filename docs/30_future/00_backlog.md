...
The "Governing loads (SELECT)" per-component tables build rows as
`row[lv.label] = round(lv.value, 2)` — dropping `LoadValue.units`, the mandatory
`-ULT` marker and the SF column, and printing literal "None"/NaN in the sparse
cells where components have different label sets. This is the page an engineer
screenshots into a design review, and it is the least trustworthy-looking one
while contradicting the lower per-section tables (which render via `report.py`
correctly). **Not merely cosmetic:** `_display_loads` only does the Imperial→SI
conversion — it never applies the limit→ultimate factor — so the headline is
showing **unmarked LIMIT loads as if they were the deliverable**, violating the
ultimate-output contract on a consolidation page.

**Root cause / reuse.** The correct render boundary already exists in `report.py`
(`_is_load_unit`, `_ult_units`, `_ult`): it scales *only* load quantities
(forces/moments/pressures) to ultimate and appends `-ULT`, while leaving
dimensionless/other columns (n, CL, V in KEAS) unscaled and unmarked — exactly
what a mixed governing-loads row needs. The bug is that the SELECT headline
bypasses this boundary and hand-formats values.

**Decisions (2026-07-20, user-confirmed):**
- **Flat SF 1.5 at the render boundary.** `CriticalCondition` carries **no**
  `safety_factor` field, but every SELECT governing condition is a standard limit
  flight-load (SF 1.5 per 14 CFR 23.303). Apply `constants.ULTIMATE_FACTOR` at the
  view and show "SF 1.5"; **no** `models.py`/`io.py`/`SCHEMA_VERSION` change. The
  per-case *carrier* on `CriticalCondition` is deferred to **M4-8** (centralized
  two-layer SF policy), which is where a non-1.5 governing case would first arise —
  adding a half-measure field now would just be reworked there.
- **Fix both tables via one shared helper.** The Flight Envelope → **Critical
  Loads** tab (`flight_envelope.py:344-348`) renders the *same* governing-loads
  table with the *same* bug and is likewise unmarked. Extract one renderer and use
  it on both so they can't diverge; both render ULTIMATE.

**Implementation:**
1. **Promote the `report.py` boundary to public API.** Add thin public wrappers
   over the existing privates — `ultimate_units(units, quantity="") -> str` and
   `to_ultimate(value, units, quantity="", sf=ULTIMATE_FACTOR)` — so the ultimate
   boundary stays owned by `report.py` (CLAUDE.md: applied "once, at the
   render/export boundary"). Keep the privates; wrappers just expose them.
2. **New shared renderer.** Add `governing_loads_table(conditions, system,
   sf=ULTIMATE_FACTOR) -> pd.DataFrame` (home: a small helper in `report.py`
   returning row dicts, or `app/components.py` if it must build a `pd.DataFrame`
   — prefer `report.py` returning rows, keeping pandas in the view). For a list of
   `CriticalCondition` (one component's `conds`), it:
   - converts each condition's `loads` to display units (reuse the existing
     `_display_loads` unit-conversion path — factor it out of the two views);
   - builds one row per condition: `Condition`, `FAR`, `V-n case`, then one column
     per load label with header `f"{label} ({ultimate_units(units, quantity)})"`
     (dimensionless labels — n, CL — get no unit suffix; V gets `(KEAS)`), and the
     **value scaled** via `to_ultimate(value, units, quantity, sf)` and formatted
     with `report._fmt`;
   - appends an **`SF`** column = `_fmt(sf)` for the row (the factor applied to its
     load cells);
   - fills **absent cells with `"—"`** across the union of columns (values are
     formatted strings, so mixing "—" with numeric-looking strings is clean).
3. **Rewire both views.** `results_review.py:94-108` and `flight_envelope.py`
   `_tab_select` (:333-348) both call `governing_loads_table(...)` instead of the
   hand-built rows. Delete the duplicated `_display_loads` in favor of the shared
   one. Drop the now-misleading "Loads shown are LIMIT" implication — these render
   ULTIMATE.

**Acceptance:**
- Every load column in both governing-loads tables shows units **with `-ULT`**
  (e.g. `lbs-ULT`, `ft-lb-ULT`; SI `N-ULT`, `Nm-ULT`); dimensionless/speed columns
  (n, CL, V) show plain units and are **not** scaled or marked.
- Load values equal **limit × 1.5**; an **SF** column reads `1.5`.
- No `None`/NaN in any cell — absent values render `"—"`.
- The Results Review headline and the Flight Envelope Critical Loads tab produce
  **identical** governing-loads tables (same helper) for the same project.
- No calc/oracle/model/schema change (Appendix A oracles untouched — the calc still
  emits LIMIT; only the render boundary is corrected). Full suite green; `ruff`
  clean.
- Test (`tests/test_results_review.py` or extend `test_report.py`): feed a known
  `CriticalCondition` (GA6 governing wing case) through `governing_loads_table`;
  assert (a) a force column header carries `-ULT` and its value is `limit*1.5`,
  (b) the `n`/`CL` columns are unscaled with no `-ULT`, (c) an SF column = `1.5`,
  (d) a sparse cell is `"—"`.
- Docs synced: `PROGRAM_SPEC.md`/`03_gui_rework_plan.md` note that Results Review
  and the Critical Loads tab render governing loads as ULTIMATE via the shared
  helper; backlog→history; `CHANGELOG.md`.

**Note.** This closes the last unmarked-LIMIT deliverable surface found by the
review; it dovetails with **M4-8** (which will give `CriticalCondition` a real
per-case factor and let `governing_loads_table` read `c.safety_factor` instead of
the flat `sf` argument — a one-line change then). *(S; unblocked.)*
