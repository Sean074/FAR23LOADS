...
| 19 | **The loader accepts text polyline corners** — `io.project_from_dict` reloads string corners unchanged, so a file saved by a pre-fix build (or edited by hand) re-crashes WINGGEOM on load *(C210-7 residual; the grid-entry half of this row — lost cells, dropped Enter, swallowed minus — was one remount race, fixed in-session as C210-4, `changes/oracle-grid-stable-frame.fixed.md`; class b → 0.7.2; [build review 2026-08-23](../50_reviews/2026-08-23_c210_oracle_gui_build_review.md))* (#76) | The loader coerces or refuses a non-numeric corner, under a guard | V | S / S | — |
| 19a | **The oracle GUI renders no consistency warnings** — `validation.consistency_warnings` is the analysis contract's page-targeted entry-error channel; the main GUI calls it on every input page (weight_mass.py:410 et al.), `oracle_app`/`app_shell` consume `ConsistencyWarning` nowhere, so a fired `wing_fraction_on_wing_row` warning (three contradictory entries) survived the whole C210 build review unshown — the oracle GUI's only diagnostic surface is the results-block station warnings *(C210-35, class b → 0.7.2; [build review 2026-08-23](../50_reviews/2026-08-23_c210_oracle_gui_build_review.md))* (#82) | The oracle GUI renders `consistency_warnings(project)` filtered by page target above each form, via a shared `app_shell` renderer so the two GUIs cannot diverge (practice 3) | V | S / S | — |
| 19b | **The flap page silently omits the 23.457(b) slipstream amplification when no engine record exists** — `flap.py:161` computes the term only when `maxhp > 0`; with `Project.engines` empty the form still collects the slipstream geometry (AF, BLPROP), consumes it for nothing, and prints a critical flap load with a ~x1.4 first-order amplification absent and zero trace (the manual's own example prints x1.407) *(C210-40, class b → 0.7.2, owner ruling: "the lack of slip stream feels like it should be at minimum a warning"; [build review 2026-08-23](../50_reviews/2026-08-23_c210_oracle_gui_build_review.md))* (#83) | A `consistency_warnings` entry targeted at the Flap Loads page — owner wording: "no slipstream effect included as engine location not set, see page engine mount loads" — fired whenever the term is skipped, riding the #82 channel (practice 3: one source, both GUIs; lands with or after #82's renderer); AF/BLPROP caption their engine-record dependency | V | S / S | — |
| 19c | **A bundle-everything results download in the shared sidebar** — thirteen pages of results left the C210 build one hand-clicked download at a time; no control delivers a complete results set for a project *(C210-45, owner-raised: "a button to save a project's results, that downloads all the output files"; design settled in-session: one zip via the browser save dialog, contents = every page report + tabular CSV exports + the serialized `project.json`, button in the shared `app_shell` sidebar so both GUIs carry it; [build review 2026-08-23](../50_reviews/2026-08-23_c210_oracle_gui_build_review.md))* | A "Download results (zip)" button in the shell sidebar project block: run every page module for the current project (pages that refuse are skipped and listed in a manifest inside the zip), render via the existing report/CSV renderers, add the serialized project, deliver with `st.download_button` — **owner: build this cycle, after the live GUI session closes** | V | M / M | — |
