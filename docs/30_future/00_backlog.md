...
sites. **Cut 0.8.1 when band B1 is empty, then band B2 (0.9.0).** Before it: **0.8.0 is cut** — `v0.8.0`, 2026-08-28,
...
| 2 | **A zero lift polynomial is laundered into a 400-trip `SolverFailure` instead of refused by name** — a coefficient set whose lift polynomial is identically zero reaches `flight_envelope.balance_configs` and dies as "did not converge in 400 iterations … reached NZ=0 at alpha=41.3861 deg": CL is 0 at every alpha, so NZ never moves off 0 and no number of trips can converge, and the V-n page names no input. Same class as the #81 stall-CL refusal and `build_envelope`'s weightless-CG / tail-CP-at-datum refusals, which name the quantity and the page at the point that consumes it; `normalize()` filling `stall_cl` from `clmax_flap` is why the #81 guard does not catch it *(found 2026-08-29 diagnosing the GA6 V-n failure)* (#144) | `balance_configs` raises `MissingInputError` naming the set and the fix, beside the #81 refusal and for every writer; the guard's docstring states the ruling on an all-zero moment/drag polynomial (drag and moment may legitimately be zero, lift cannot carry the airplane); unit test from both `build_envelope` and `trim_sweep`, GA6 oracle and twin closure suites unmoved | V | M / S | — |
