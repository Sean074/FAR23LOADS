...
**Where things stand (2026-08-28):** **0.8.1 is the milestone in flight** — a
...
the blank-derive crash pair (#121/**#122**, the latter closed 2026-08-29), and **#132** — the
...
delivered CSV states neither the frame nor the application point), **#140** (the
guide's landing chapter still describes the pre-0.8.1 output) and **#142** (the
docs-hygiene pass) — folded in from the 0.8.2 band they were first tabled as
...
sites — **#144 closed 2026-08-29**, so the phantom set now refuses by name at
the consumer and #143's repro reads as the input error it is. **Cut 0.8.1 when band B1 is empty, then band B2 (0.9.0).** Before it: **0.8.0 is cut** — `v0.8.0`, 2026-08-28,
...
|---|---|---|---|---|---|
| **B1 — 0.8.1: released-defect correction, with the 2026-08-29 independent review's documentation band folded in (re-cut 2026-08-29, second)** ||||||
| 1 | **flight_envelope view crashes on a blank `full_down_aileron_deg` — the note-36 derive default is unrenderable in the main GUI** — `select_input.full_down_aileron_deg` defaults to `None`, a documented, meaningful state (the registry's sentinel-default class, #98/C210-49) the oracle GUI renders and SELECT derives, but the view dies with `TypeError: float(...)` on `NoneType`; baron_58 enters 14.0 (= its aileron limit) to keep the suite green, so **no shipped fixture exercises the blank and no guard covers it** *(found 2026-08-27 building the guide's baron_58, #96 stage 2)* (#121) | The view renders the blank as the oracle GUI does (empty widget + the derived value stated) or at minimum survives it; the rule-4 sweep of `app/views/` for the same `float(field)` pattern over other sentinel-default registry fields; the blank-field case added to the views dirty-flag/smoke sweep. **The `app/views/` freeze stays shut:** 0.8.1 takes the *at minimum survives it* half — the named refusal, the rule-4 `float(field)` sweep and the smoke case — and leaves the render-it-as-the-oracle-GUI-does half to #29's freeze lift, where the layout question belongs (re-cut 2026-08-28) | V | S / S | — |
| 3 | **One stray touch attaches a zero-coefficient flaps-down set, and there is no way to remove it** — the oracle GUI renders the LANDING block from a blank record held detached in `_PENDING`; a visit attaches nothing (that contract holds), but one interaction with any widget in the block makes the record non-blank and `commit_pending` attaches the whole zero-polynomial set, un-checking does not detach it, `refresh_derived` → `normalize()` fills `stall_cl` from `clmax_flap` so it passes the #81 guard, and the phantom set hangs the balance and **saves into the project file**. The inverse of the #51 data-loss class — silent data *gain* — defeating the OG-F "a page visit must not dirty a project" posture one gesture later, taking Flight Envelope and SELECT down with an opaque solver error *(found 2026-08-29 diagnosing the GA6 V-n failure)* (#143) | A stray touch of an unattached record block attaches nothing — attachment by a deliberate, named gesture, the row add/delete posture — **or** an attached-but-blank set is treated as absent by its consumers; either way the GA6 repro (Aero page → check → uncheck → V-n) runs clean. A non-list record the user can create gets a deletion control that names what it removes (the #88/#72 contract). AppTest regression on the real path beside `test_the_aero_page_fills_the_stall_cl_it_was_given_field_by_field`. **Fix site is `oracle_app/`, so the `app/views/` freeze is untouched** | V | M / S–M | #144 (its refusal makes the repro legible) |
| 4 | **The delivered CSV states neither the frame nor the application-point name** — the landing CSV carries the point numerically (x/y/z per gear) but `point_name` (axle vs ground contact) and the "with respect to airplane datum" words live in the condition note and the GUI captions, both of which the CSV drops; a standalone consumer cannot tell case 1 acts at the axle except by comparing coordinates to geometry. Channel to decide in the issue: a landing methods-preamble line (the channel exists) and/or an emitted point-name row (#141) | The CSV self-contained: frame and application point stated in-band; `PROGRAM_SPEC.md` §landing + history fragment per tier M | E | M / S | — |
| 5 | **Guide landing chapter describes the pre-0.8.1 output** — `docs/60_guide` has zero diff across v0.8.0..dev/v0.8.1; `14_landing_loads.md` §"Results on this page" still describes the primed-only output: no body-frame three-wheel deliverable, no axle/contact split (note 39 AP-1), no two-frame statement or CSV/report split (note 38 GF-6/GF-7), no NR/NV/ND or fuselage-axis angle; `03_conventions.md` checked for a frames statement in the same pass (#140) | `14_landing_loads.md` (and `03_conventions.md` if touched) at the final definition, citing the register — no development trail | V | S / S | #141 (describes the shipped CSV) |
| 6 | **Development-history narration in standard/theory docs; superseded-entry banner contradiction** — dated changelog narration audited out of the final-definition docs (owner's call per line; some may stay as measured-effect evidence): `PROGRAM_SPEC.md` "green throughout the year", `CONVENTIONS.md` "until 2026-08-29 … 524,302 lb-in", `theory_sources.md` "From 2026-08-15 this row recorded…", `balanced_cases.md` "Updated 2026-08-29, twice" banner; same pass, one defect class: the superseded declined entry in `02_approved_corrections.md` keeps an inner banner promising "converts in place" against the outer "kept verbatim" banner — reconciled (#142) | The audited docs at final-definition style; anything relocated lands in the history fragments, nothing lost | V | S / S | — |
| 21 | **The GUI release gate proves boot, not use** — `scripts/smoke_test.sh` (RELEASE_PROCESS §3.5) boots both entry points and checks the root page answers 200; #143 needs load-example → Aero page → one touch → V-n page, a defect two pages downstream of an interaction that no boot check can reach, and the schema-58 stale-process incident was likewise only visible by walking a loaded example through the workflow. Per-page AppTest coverage is extensive; the cross-page journey is the missing piece *(filed 2026-08-29 from the post-0.8.0 escape assessment)* (#145) | An AppTest journey test in the fast CI gate and §3.5: every bundled example loaded, every `workflow.py` page visited in order (the nav SSOT, so the journey cannot rot), one interaction per editable block, every module run — no page raises, every module runs clean or refuses with a named `MissingInputError`, and the project dict is byte-identical wherever no edit was intended (OG-F across the whole journey — the assertion that catches the #143 silent-data-gain class); §3.5 names it and gains the short scripted manual walkthrough as the second line; the boot smoke unchanged | V | M / M | #143 (the journey's first regression target) |
| 22 | **Oracle provenance and gate independence are unstated** — the worst 0.8.0 escapes (#133, #139, the 2803-lb ga6 fixture) share one mechanism: the oracle silently shrank — an illegible column became a missing oracle, a back-solved input posed as one, and a gate re-derived the rule it was checking ("the gate … was making the correction itself"); "oracle test exists, ±0.1%" is binary where it needs provenance *(filed 2026-08-29 from the post-0.8.0 escape assessment)* (#146) | Two rules in CODE_REVIEW_PROCESS Step 3 + `theory_sources.md`: every oracle cell states its provenance (transcription / OCR / absent-because-illegible; a back-solved input disqualified as the oracle for anything downstream of what it was solved from), and no gate re-derives the rule it checks (G-AP-2's "two copies of one rule cannot disagree", promoted to a named standard rule); one bounded provenance sweep of `theory_sources.md` listing any family still running on internal identities only; no calc change | V | S / S | — |
...
| 14 | Export deck-writing primitives out of `sbeam_bridge.py` (CH-4) (#15) | `_fmt`/`_sf_str`/`_stamped`/`_MAT1_*`/`_PBAR_*` in a shared module; the four private cross-imports gone | V | S / S | — |
| 15 | Dead code (CH-5) (#16) | Delete `write_balanced_deck`, `write_conm2_fragment`, `write_mass_check_deck`, `all_checks`; demote the ~12 no-consumer public names | V | S / S | — |
| 16 | Calc-side function size (CH-8) — `build_lra_model` (336 lines), `landing_reactions` (200) (#17) | Split when touched; **the view functions wait for the GUI review (#29)** | V | S / S | — |
| 17 | Review 2026-08-10 unscheduled findings m3–m13, m15–m18 + NITs *(defect sweep)* (#18) | Swept opportunistically (practice 4) or promoted individually | V | S / S–M | — |
