...
|---|---|---|---|---|---|
| **B — 0.8.0: oracle-GUI development — extended 2026-08-27 with the landing load-factor defect and its sweep finding (owner ruling; cut when empty, RELEASE_PROCESS §4)** ||||||
...
| 7 | **The landing load factor is entered as NLG, so the wing lift factor `L` is inert on the gear reaction and the page reports an N the reactions were not computed from** — `VMP = ½·NLG·W·AP/DP` reads the entered `gear_load_factor` and nothing else, so changing `L` moves no wheel load (owner-reproduced on `ga6_normal`, 2026-08-27), while the page and `run()` display LGFACTOR's *energy* N (ga6: 3.0972) instead of the value the deck actually runs at (2.5 + 0.667 = 3.1670). `PROGRAM_SPEC.md` §LANDLOAD records the split as intent; this row rules it a defect. **A first-order defect on shipped output — under the 2026-08-24 ruling 1 the owner may pull it into a 0.8.x patch ahead of this band.** Design note **37** (LF-1…LF-12, G-LF-1…G-LF-6), AGREED 2026-08-27 (#123) | `N` and `L` become the inputs and `NLG = N − L` is derived, reported and never enterable; `gear_load_factor` deleted for `airplane_load_factor: Optional[float]` (schema v56 → v57, semantic hop `N = NLG_old + L`); `L` uncapped for the FAR 25 basis with the FAR defaults captioned; `N ≤ L` refused; the 23.473(g) floors block in FAR 23 and warn in concept; guidance and a seeded-clearable `N` in **both** GUIs. Oracle-neutral by construction — p236 untouched (LF-3), p230 held by ga6 at `N = 3.167` (LF-9) | E | L / M | note 37 (AGREED) |
