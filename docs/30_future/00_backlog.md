...
| 1 | **The %MAC ↔ fuselage-station Tool reinterprets its entered station on a unit switch** — the station widget ([sidebar.py:366](../../app_shell/sidebar.py)) is a bare `st.number_input` seeded with a converted length and keyed without the system suffix `components.number_input_name` appends on the converted path, so Streamlit's retained state outvotes `value=` and the same number is read as inches then as millimetres. Reproduced headless on `ga6_normal`: 63.641 → **0.00 % MAC** in Imperial, the identical widget → **−88.29 % MAC** after toggling to SI. The **only** converted number input in either GUI outside `unit_number_input` (rule-4 scan of every `number_input(` site in `app_shell/`, `oracle_app/`, `app/views/` — `engine_mount.k()` and `flight_envelope._num` both suffix correctly). Display-only, so nothing is stored and no load moves *(production-release review 2026-08-27 §3.4; the Tools row #80's own new code)* (#126) | The station input goes through `unit_number_input(..., kind="length")` like every other converted number, and a **drift guard** drives the Tools panel across a unit toggle asserting the %MAC answer is invariant — practice 3's guard, which #80 shipped without: the four existing Tools tests all run Imperial | V | S / S | — |
| 2 | **The pre-release smoke gate never launches the oracle GUI** — `RELEASE_PROCESS.md` §3.5 is a hard gate and `scripts/smoke_test.sh:56` starts `app/Home.py` and nothing else, so the release whose headline deliverable is the second front-end has no gate that boots it under a real server. Narrower than it reads: `Oracle.py` **is** exercised in-process by `AppTest` (`test_app_shell.py:508`, `test_oracle_gui.py:1252`), so `set_page_config`, `st.navigation` and the sidebar context manager are covered; what nothing covers is the server boot and the `sloads-oracle` console script actually launching (`test_oracle_gui.py:1273` asserts only that `entry_point_path()` resolves) *(production-release review 2026-08-27 §3.2)* (#127) | `smoke_test.sh` smokes both entry points (an entry-point argument, invoked twice, is the cheapest shape), §3.5's checklist line names both, and the `sloads-oracle` launcher is run rather than path-checked | V | S / S | — |
| 3 | **A design note's status line can claim work is unbuilt after it has shipped** — [note 32](32_oracle_gui_note.md) §Status still reads *"everything else is unbuilt"* for the oracle GUI whose OG-A…OG-F shipped 2026-08-18/20 and carried eight further issues this band; [note 35](35_taildist_aero_state_note.md) still reads *"Nothing below is built yet"* for work that shipped as #100 on 2026-08-27 (`changes/taildist-aero-state.changed.md`, `taildist.py:84`). Note 34 (*"AGREED …; BUILT 2026-08-27 (#96 …)"*) and notes 36/37 (SHIPPED) are the model. It blocks the cut rather than trailing it: `RELEASE_PROCESS.md` §4 step 3 rolls these notes into `docs/40_history/` at the cut, so an "unbuilt" claim enters the permanent record of the release that built it *(production-release review 2026-08-27 §3.3)* (#128) | The two status lines state what shipped, **and** the structural half (practice 3): `tests/test_doc_currency.py` gains a guard that a design note whose issue is closed — or whose `changes/*.md` fragment exists — cannot still assert unbuilt work. The guard is the row; the two corrections are its first sweep | V | S / S | — |
| 4 | **`use_container_width` survives in `app_shell/` on an unbounded Streamlit floor** — eleven sites (`sidebar.py` ×7, `project_state.py` ×4) while `oracle_app/` has already migrated to `width="stretch"`. Streamlit 1.58 documents the parameter as *"deprecated and will be removed in a future release"* on both `st.button` and `st.dataframe`, and [`pyproject.toml:29`](../../pyproject.toml) pins `streamlit>=1.36` with **no upper bound** — so the release that removes it breaks Open / Save / Download / Build-results-zip in **both** GUIs for a fresh install, with no version constraint to stop it. Currently parked inside the main-GUI omnibus `L-8e` ([02_parked.md](02_parked.md)); by the 2026-08-24 re-cut's rule 2 (rows are placed by fix site) the shared-shell half belongs here *(production-release review 2026-08-27 §3.7; owner decision §5.4 — split out or ride to 0.9.0)* (#129) | The migration is taken **whole** — the eleven shell sites and the fourteen `app/views/` ones — because an upstream removal breaks both front-ends in the same release, so splitting it by GUI leaves half a bomb armed (owner ruling 2026-08-28, review §5.4); the `L-8e` bullet drops it entirely, pointing here; and the runtime dependency floor gains a stated ceiling policy (a tested upper bound, or an explicit decision recorded that there is none) | V | S / S | — |
| 6 | **The max-continuous-HP precedence rule is written twice** — `weight_estimate.resolve_max_continuous_hp` ([weight_estimate.py:345-359](../../sloads/modules/weight_estimate.py)) and again inline in the view ([weight_mass.py:206-208](../../app/views/weight_mass.py)); two copies of one precedence, currently agreeing — the drift practice 3 forbids, same shape as the `_ground_angle` duplication `landing.py` calls out in prose *(adjacent finding of the note 37 §4 sweep, filed 2026-08-27 under rule 5)* (#124) | The view reads `resolve_max_continuous_hp` — one owner for the precedence, the inline copy deleted; a drift-guard test if a second consumer remains | V | S / S | — |
