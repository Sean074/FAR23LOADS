...
| 16 | **The suite-runtime revisit clause has tripped — split the whole-pipeline-per-assertion tests** *(CR-D-6, filed from #46; hygiene)* (#92) | The tests that re-run the whole pipeline once per assertion — `test_case_ids` plus the four ~20–37 s deliverable-units/balance tests share that one shape — reduced to one pipeline run per example with the assertions reading its result, as the 2026-08-16 case was fixed. Measured at the trip: 269 s parallel (671 s serial) against the clause's ~2 min, slowest test 62.3 s (`test_case_ids::test_the_index_row_states_the_condition_its_cards_were_computed_at`, the parallel critical-path floor) against its ~30 s, 15 tests over 16 s. **No `slow` marker** — `00_program_overview.md` §Testing states why. Done when `--durations` puts the critical path back inside the clause's own thresholds; the stale claim on the standard page was already corrected with #46, so what is open here is the work, not the wording | V | S / S–M | — |
